...
const lastLinkedPrsFetch = new TtlMap<string, number>(2_000, LINKED_PRS_STALE_MS);
const inFlightLinkedPrs = new TtlMap<string, Promise<void>>(2_000, LINKED_PRS_STALE_MS);
...
// Keyed by repo full name — bounded at 500 repos; TTL matches the longest
// staleness window (COMMENT_STALE_MS) so entries never linger longer than needed.
const inFlightIssues = new TtlMap<string, Promise<void>>(500, COMMENT_STALE_MS);
const inFlightPulls = new TtlMap<string, Promise<void>>(500, COMMENT_STALE_MS);
const inFlightComments = new TtlMap<string, Promise<void>>(500, COMMENT_STALE_MS);
const lastCommentsFetch = new TtlMap<string, number>(500, COMMENT_STALE_MS);
